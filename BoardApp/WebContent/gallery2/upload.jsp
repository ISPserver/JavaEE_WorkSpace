...
/*파일 업로드 컴포넌트의 종류엔 여러가지가 있지만, 그 중 아파치의 공식 업로드 컴포넌트를 사용해본다.*/
	...
	//업로드 객체를 생성해주는 팩토리 객체: 주로 설정 담당(서버와 저장결로, 파일 용량제한)
	DefaultFileItemFactory itemFactory = new DefaultFileItemFactory();
	itemFactory.setRepository(new File(saveDir));//저장할 위치
	itemFactory.setSizeThreshold(maxSize);//용량을 지정한 크기로 제한
	
	//이 객체가 실제 업로드를 수행함
	ServletFileUpload upload = new ServletFileUpload(itemFactory);//설정 정보를 생성자의 인수로 전달
	
	//FileItem은 클라이언트의 전송 정보 하나 하나를 의미한다. 즉 html에서 input box, file 컴포넌트 등
	//우리의 경우 input type="text"가 Fileitem에 담기고,
	//input type="file"가 Fileitem에 담김
	...
	for(FileItem item : items){
		...
		if(!item.isFormField()){//텍스트박스가 아닌것만 업로드 처리
			//메모리상의 이미지 정보를 실제 물리적 파일로 저장하기 (업로드 처리)
			//새롭게 생성할 파일명
			...
			out.print("저장된 경로:"+file.getAbsolutePath()+"<br>");
			out.print("업로드 파일크기:"+item.getSize()+"<br>");
		}
	}
%>